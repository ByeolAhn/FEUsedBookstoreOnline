...
const handleDeleteAccount = () => {
    const confirmation = window.confirm('Are you sure you want to close your account?');
    if (confirmation) {
  ...
  const renderProfileForm = () => {
    if (isLoading) {
      ...
      const { userId, username, email, role, phoneNumber, address, lastLogin } = userProfile;
    ...
    <form style={{ width: "50%" }}>
      ...
      <h1>MY PROFILE</h1> 
      <h4><i className="fas fa-edit ml-2"></i></h4>
      ...
      <label><b>My Id:</b></label>
        ...
        <input type="text" value={new Date(lastLogin).toLocaleString()} disabled />
      </div>
    </form>
  </div>
